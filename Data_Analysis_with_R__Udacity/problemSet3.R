...
# Q4
summary(diamonds$price)
# Price itself has an exponential distribution; median 2401, mean 3933, min 326, max 18820
...
for (x in possibleAnswers) {
  if (length(diamonds$carat[diamonds$carat == x]) > 2000) {
    print(x)
  }
}
...
#   print(years)
  ...
  ### cannot get boxplot to accept a vector per label rather than a long (label, value) list ### 
  #   - could look at different boxplot functions
  flatList <- list()
  ...
  twoEntryList[,2] <- as.double(twoEntryList[,2])
